    ...
    for step in steps.drain(..) {
        match step {
            Step::Set {
                ...
                let result = store.set(txn_id, None, key.as_bytes(), val.as_bytes().to_vec());
...
fn test_set_and_get_uncommitted_same_txn() {
    run_test(vec![
        ...
        Step::Get {
            txn_id: 0,
            key: "foo",
            expect: Ok(Some("bar".to_string().into_bytes())),
        },
        Step::CommitTxn {
            txn_id: 0,
            expect: Ok(()),
...
fn test_update_and_get_committed() {
    run_test(vec![
        ...
        Step::Get {
            txn_id: 2,
            key: "foo",
            expect: Ok(Some("updated".to_string().into_bytes())),
        },
        Step::CommitTxn {
            txn_id: 2,
            expect: Ok(()),
...
fn test_commit_multiple_changes() {
    run_test(vec![
        ...
        Step::Get {
            txn_id: 2,
            key: "foo",
            expect: Ok(Some("bing".to_string().into_bytes())),
        },
        Step::Get {
            txn_id: 2,
            key: "baa",
            expect: Ok(Some("biz".to_string().into_bytes())),
        },
        Step::CommitTxn {
            txn_id: 2,
            expect: Ok(()),
...
fn test_phantom_insert_then_read_validation() {
    run_test(vec![
        ...
        Step::Get {
            txn_id: 1,
            key: "foo",
            expect: Ok(Some("phantom".to_string().into_bytes())),
        },
        Step::CommitTxn {
            txn_id: 1,
...
fn test_failed_commit_reverts_delete() {
    run_test(vec![
        ...
        Step::Get {
            txn_id: 5,
            key: "bar",
            expect: Ok(Some("revert".to_string().into_bytes())),
        },
        Step::CommitTxn {
            txn_id: 5,
            expect: Ok(()),
